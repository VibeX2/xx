...
//                            int idItemNotSell[] = {14,15,16,18,19,20, 441,442,443,444,445,446,447, 76,188,189,190, 225, 85, 993};
                                int idItemNotSell[] = {188, 189, 190, 18, 19, 20, 441, 442, 443, 444, 445, 446, 447, 188, 189, 190, 225, 188, 189, 190, 18, 19, 20, 76, 188, 189, 190, 18, 19, 20, 85, 993};
                                int percentDrop = Util.nextInt(0, 32);
...
//                                if (percentDropKichHoat < 5 && map.MapSetKichHoat()) { //ITEM ROT LA ITEM KICH HOAT
                                if (percentDropKichHoat < 50 && p.checkFullSetThienSu() && map.MapSetKichHoat()) { //ITEM ROT LA ITEM KICH HOAT
                                  ...
                                  itemROI.itemTemplateID = (short)itemROI.itemMapID;
                                    ...
                                    if (percentDrop < 3 && map.MapCell() && p.useMayDoCapsule) { //ITEM ROT KHI DUNG RADA DO CAPSULE
//                                int idItemNotSell2[] = {16,18,19,20, 441,442,443,444,445,446,447, 76,188,189,190, 225,16,18,19,20, 441,442,443,444,445,446,447, 76,188,189,190, 225, 380};
                                    int idItemNotSell2[] = {225, 18, 19, 20, 441, 442, 443, 444, 445, 446, 447, 188, 189, 190, 225, 225, 18, 19, 20, 441, 442, 443, 444, 445, 446, 447, 76, 188, 189, 190, 225, 380};
...
//                                }
                            ...
                            if (mob.template.tempId != 0 && percentDrop2 < 3) {
                                int idItemNotSell2[] = {188, 189, 190, 18, 19, 20, 441, 442, 443, 444, 445, 446, 447, 188, 189, 190, 225, 188, 189, 190, 18, 19, 20, 76, 188, 189, 190, 18, 19, 20};
                                int id2 = Util.nextInt(0, 31);
                    ...
                    for(Mob mob: mobs) {
                        if (Math.abs(player.x - mob.pointX) < templateSkillUse.dx && Math.abs(player.y - mob.pointY) < templateSkillUse.dy && !mob.isDie) {
                                ...
                                if(mob.isDie) {
                                    ...
                                    if(mob.template.tempId != 0) {
                                        int idItemNotSell[] = {188,189,190,18,19,20, 441,442,443,444,445,446,447,188,189,190, 225,188,189,190,18,19,20,76,188,189,190,18,19,20};
...
//        if(_detu.tiemNang > 0 && (_detu.hpGoc < _detu.getHpMpLimit() || _detu.mpGoc < _detu.getHpMpLimit() || _detu.damGoc < _detu.getDamLimit())) {
        ...
        if (_mob.isDie) {
            ArrayList<ItemMap> itemDrop = new ArrayList<>();
            if(_mob.template.tempId != 0) {
                int idItemNotSell[] = {188,189,190,18,19,20, 441,442,443,444,445,446,447,188,189,190, 225, 76,188,189,190,18,19,20,188,189,190,18,19,20};
                int percentDrop = Util.nextInt(0, 10);
                int percentDropItemGod = Util.nextInt(0, 40000);
                int perTA = Util.nextInt(0, 5000);
...
//                                    _broly.hpFull = (int)(_broly.hpFull*2);
    ...
    //                        }
     ...
     public void loadBEE(Boss _broly) {
        haveBROLY = true; //check xem map da co broly hay chua
        Message msg;
        try {
            loadInfoBoss(_broly); //tha broly theo server roi khong can ve cho all player trong map nua
...
//                Util.log("TI LE NE DON: " + _bTarget.getPercentNedon());
    ...
    public void loadBossNoPet(Boss _BOSS) {
        ...
        try {
            ...
            } else if((_BOSS._typeBoss >= (byte)52 && _BOSS._typeBoss <= (byte)54)) {
                Service.gI().sendThongBaoServer("BOSS " + _BOSS.name + " vừa xuất hiện tại " + map.template.name);
            }
...
//                        timerBOSS.cancel();
    ...
    //                        haveBROLY = false;
                            if(_BOSS._typeBoss == 3) { //COOLER 1 DIE
                                ...
                                timerBLACKXuatHien.schedule(ttBLACKXuatHien, 90000);
                            }else if(_BOSS._typeBoss == 51) { // bee 1
                                Timer timerBLACKXuatHien = new Timer();
                                TimerTask ttBLACKXuatHien = new TimerTask() {
                                    public void run()
                                    {
                                ...
                                timerBLACKXuatHien.schedule(ttBLACKXuatHien, 90000);
                            }else if(_BOSS._typeBoss == 51) { // bee 2
                                Timer timerBLACKXuatHien = new Timer();
                                TimerTask ttBLACKXuatHien = new TimerTask() {
                                    public void run()
                                    {
                                ...
                                timerBLACKXuatHien.schedule(ttBLACKXuatHien, 90000);
                            }else if(_BOSS._typeBoss == 51) { // bee 3
                                Timer timerBLACKXuatHien = new Timer();
                                TimerTask ttBLACKXuatHien = new TimerTask() {
                                    public void run()
                                    {
                                ...
                                timerBLACKXuatHien.schedule(ttBLACKXuatHien, 90000);
                            }else if(_BOSS._typeBoss == 53) { // ky lan
                                Timer timerBLACKXuatHien = new Timer();
                                TimerTask ttBLACKXuatHien = new TimerTask() {
                                    public void run()
                                    {
                                        int idMap = 0; 
                                        int IDZONE = Util.nextInt(1, map.area.length);
                                        short xBlack = 228;
                                        if(idMap == 91) {
                                            xBlack = 1296;
                                        }
                            ...
                            }else if(_BOSS._typeBoss == 53) { // ky lan 1
                                Timer timerBLACKXuatHien = new Timer();
                                TimerTask ttBLACKXuatHien = new TimerTask() {
                                    public void run()
                                    {
                                        int idMap = 7; //index 91 la map 92, index 92 la map 93
                                        int IDZONE = Util.nextInt(1, map.area.length);
                                        short xBlack = 228;
                                        if(idMap == 91) {
                                            xBlack = 1296;
                                ...
                                timerBLACKXuatHien.schedule(ttBLACKXuatHien, 120000);
                            }else if(_BOSS._typeBoss == 53) { // ky lan 2
                                Timer timerBLACKXuatHien = new Timer();
                                TimerTask ttBLACKXuatHien = new TimerTask() {
                                    public void run()
                                    {
...
//                                }
                    ...
                    if(_charTarget != null) {
                        if(_BOSS._typeBoss < (byte)7) {
                            ...
                            chat(_BOSS, "Haha Tao ngu vcl!");
                            chat(_BOSS, "Đố ai ngu được bằng tao!");
                        }
                    }
                }
            };
